...
Likely you will feel ripped off and a bit angry at the end of this talk as I will not be telling you anything new.
}
...
With more and more devices being connected the risk will only increase.

Quiz (no prize): how many lightbulbs are in this room?
}
...
\item turn device on
\item replace device when it breaks
\end{enumerate}
...
ODMs also told me that once a chipset vendor releases a new SDK they (chipset vendor) often spend months doing all kinds of tests (security does not happen to be one of them), so there is some reluctance to frequent updates.
}
...
\frame{
\frametitle{Using security information from CVEs}
...
Challenge: CVE information is often unstructured, so this means a lot of manual processing.

A simple keyword or pattern search in binaries should also give hints of possible bugs.
}

\frame{
\frametitle{Analysing source code to search for smells}
...
By recording information about vulnerable firmwares/binaries and comparing new ones (checksums, bsdiff, identifiers) to this information it will become easier to find vulnerabilities in devices that were not mentioned in for example a CVE report.
}
...
The project started recently, so not everything is done yet. Finished (or almost finished) are:
...
\begin{itemize}
\item comparing binaries using checksums, bsdiff, identifiers
\end{itemize}

In the next year I will work on the other functionality.
}
...
\frame{
\frametitle{Feeling ripped off?}
If you feel ripped off by this talk and don't want me to ever give this again there are a few things you can do:

\begin{itemize}
\item complain to your vendors about security bugs so they become more aware
\item actively help hunt down security bugs in frequently used components of embedded devices
\item write more secure software
\item inform others
\end{itemize}
}
